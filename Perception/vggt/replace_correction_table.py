...
from Motion_Planning.Manipulation.Skill_Flexiv import env_build_from_vision,skill_database
from Motion_Planning.Manipulation.Skill_Flexiv.Planning_tools.get_auto_grasp_orientation import auto_orientation
...
def get_grasp_in_base_flexiv(bestman, T_grasp_in_camera):

    T_camera_in_eef = np.array(bestman.cfg.Manipulation.T_camera_in_eef)
    bestman.update_robot_states()
    tcp_pose = bestman.robot_states.tcpPose 
    orientation_quat = tcp_pose[4:7] + [tcp_pose[3]]
    rotation = R.from_quat(orientation_quat)
    rotation_matrix = rotation.as_matrix()
    T_eef_in_base = np.eye(4)
    T_eef_in_base[:3,:3] = rotation_matrix
    T_eef_in_base[:3, 3] = np.array(tcp_pose[:3])

    T_grasp_in_base = np.dot(T_eef_in_base, np.dot(T_camera_in_eef, T_grasp_in_camera))

    return T_grasp_in_base
...
def capture_images(pipeline, align, depth_scale, output_dir="/home/yan/BestMan_Chemistry_Test_SONG/Perception/vggt/captured_images"):
    ...
    return color_path, depth_16bit_path
...
def main():
    bestman = Bestman_Real_Flexiv()
    bestman.initialize_robot()
    bestman.connect_gripper()

    # Insert 'Coordinate' module to calculate grasping pose in robot's base frame
    
    position = [0.4279,0.2124,0.4066]
    orientation = [ 151, -5, 128]
    prepare_pose = Pose(position, orientation)




    bestman.move_eef_to_goal_pose(prepare_pose, maxLinearVel=0.15, maxAngularVel=math.radians(15))
    bestman.wait_for_eef(prepare_pose)
    time.sleep(2)
    ...
    color_path, depth_16bit_path = capture_images(pipeline, align, depth_scale)
    ...
    image_names = [color_path] 
    depth_image_path = depth_16bit_path
    ...
    if np.linalg.norm(rotation_axis) < 1e-6:
        ...
        compensated_world_dis = center_list[0]

        T_grasp_in_camera = np.array([[-1.0,  0.0, 0.0, compensated_world_dis[0]],
                                        [ 0.0, -1.0, 0.0, compensated_world_dis[1]],
                                        [ 0.0,  0.0, 1.0, compensated_world_dis[2]],
                                        [ 0.0,  0.0, 0.0, 1.000]])
        

        # Insert 'Coordinate' module to calculate grasping pose in robot's base frame
        T_grasp_in_base = get_grasp_in_base_flexiv(bestman, T_grasp_in_camera)
        
        position = T_grasp_in_base[0:3, 3]
        position[2] += 0.08
        orientation = [ 0.0, 180.0, 0.0]
        move_pose = Pose(position, orientation)
        ...
        D_pre = 0.08  #0.15
        D_ret = 0.10  #0.20
        grasp_pose_list = [position, orientation]


        

        skill_flexiv_database = skill_database.SkillFlexivDatabase()
        
        # running on real robot
        # when you decide to execute the task with real robot, call the following function
        skill_flexiv_database.grasp(bestman, 
                            grasp_pose_list, 
                            approaching_dir=approaching_dir, 
                            retracting_dir=retracting_dir, 
                            D_pre=D_pre, 
                            D_ret=D_ret, 
                            force=10, 
                            grasp_speed=100)
        ...
        bestman.move_eef_to_goal_pose(move_pose, maxLinearVel=0.15, maxAngularVel=math.radians(15))
        bestman.wait_for_eef(move_pose)
        time.sleep(2)
        ...
        # understanding of the parameter definition and choices require understanding of the skill implementation. 
        # It is recommanded to look into the skill implementation in the 'Manipulation' library to avoid any miss understanding and harzard caused by any misuse of the skill.

        region = 'experiment region'
            
        D_ret = 0.2 # avoid object collision
        D_pre = 0.05 # avoid object collision
        X, Y, Z = [0.65, 0, 0.1]
        position = [X, Y, Z]
        orientaiton = auto_orientation(position, constrained_pitch=bestman.constrained_grasping_pitch)
        if  region == 'collision_free region':
            orientaiton[2] = 90
        elif region == 'experiment region' or region == 'complete region':  #DELETE!!!!
            orientaiton[2] = 90
        
        place_pose_list = [position, orientaiton]
        if region == 'preparation region':
            retracting_dir = 'front'
        elif region == 'experiment region' or region == 'complete region':
            retracting_dir = 'top'
        elif region == 'collision_free region':
            retracting_dir = 'iso'
        elif region == 'wasted region':
            retracting_dir = 'iso'

        # running on real robot
        # when you decide to execute the task with real robot, call the following function

        skill_flexiv_database.place(bestman, 
                            place_pose_list, 
                            retracting_dir=retracting_dir, 
                            D_ret=D_ret)
