...
class CEDataset:
    def __init__(self, img_mode="use_base64", cfg=None , client= None):
        # 接受外部传入的 cfg
        self.cfg = cfg
        if client==None:
        ...
        self.image_path = "/home/yan/BestMan_Chemistry_Test_SONG/Perception/Object_detection/Lang_SAM/output_objects"
        self.load_json_path = "/home/yan/BestMan_Chemistry_Test_SONG/Perception/Object_detection/Lang_SAM/output_objects/objects_metadata.json"
        self.save_path = "/home/yan/BestMan_Chemistry_Test_SONG/Perception/Object_detection/Lang_SAM/output_objects/objects_gpt.json"
    ...
    def analyze_image(self, base64_image=None, other_prompt = None, url_link=None):
        ...
        messages = [
          ...
          "type": "text",
          "text": f"The 2D bounding box is {str(other_prompt)}" 
        }] },
        ]
    ...
    def manual_gpt_label(self,object_name,object_info,manual_label):
        image_name = str(object_name)+".png"
        image_path_new = os.path.join(self.image_path, image_name)
        box = object_info["box"]
        start = time.time()
        image = self.encode_image(image_path_new)
        # Analyze the image
        text = self.analyze_image(base64_image=image, other_prompt=box)
        ...
        try:
            ...
            for container in parsed_json.get("containers", []):
                ...
                container["bounding box"]=box
                container["name"] = manual_label

            for chemical in parsed_json.get("chemicals", []):
                chemical["source"] = image_path_new
                chemical["bounding box"]=box
                chemical["name"]=manual_label

            for others in parsed_json.get("others", []):
                others["source"] = image_path_new
                others["bounding box"]=box
                others["name"]=manual_label

            # Save the updated response
            self.save_json_response(json.dumps(parsed_json, indent=4), self.save_path)
    ...
    def create_gpt_label(self,object_name,object_info):
        image_name = str(object_name)+".png"
        image_path_new = os.path.join(self.image_path, image_name)
        box = object_info["box"]
        start = time.time()
...
if __name__ == "__main__":
    ...
    with open(llm_ce.load_json_path, "r") as f:
        data = json.load(f)
    #print(data['object5'])
    ...
    for object_name, object_info in data.items():
        llm_ce.create_gpt_label(object_name,object_info)
    sorted_objects,item_info_dict = llm_ce.gpt_oupput_process()
