...
def ineraction_policy_inference(camera_hand,camera_overhead, bestman,stage1segmentation,stage2editing,predictor,model_savg,model_va,gripper_open_flag):
    ...
    while True:
        ...
        if  len(model_va.predict_action_queue)<model_va.policy.horizon-model_va.policy.n_action_steps+2:
            ...
            normalize_eff_angular = 0 if eff_gripper_width<0.066 else 1 
        ...
        inference_gripper_width=inference_action[-1]*2 #recover the normal scale
        if gripper_open_flag==0 and inference_gripper_width<0.066:
            gripper_open_flag = 1
...
if USE_SAM2:
    ...
    # cv2.imwrite("/home/liusong/ProgramFiles/BestMan/Dataset/Images/TrashSweep.png",img)
    # cv2.imshow("img",img)
    # cv2.waitKey(0)


    frame = cv2.imread("/home/liusong/ProgramFiles/BestMan/Dataset/Images/TightenContainer.png")
    ...
    points = np.array([[448, 350],[454,336],[440,356]], dtype=np.float32)
    ...
    points = np.array([[351, 346],[356,376],[353,404]], dtype=np.float32)
    labels = np.array([1,1,1], dtype=np.int32)
    _, out_obj_ids, out_mask_logits = predictor.add_new_prompt(
        frame_idx=ann_frame_idx, obj_id=ann_obj_id, points=points, labels=labels
    )
