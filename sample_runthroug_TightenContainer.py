...
for i in range(50):
    ...
    while True:
        try:
            grasp_pose = [mouse_base_3d_points[0]-np.array([0,0.01,0.003])+np.array([0,0,new_red_gripper]), standard_quaternion] 
    ...
    def quat_to_yaw(quaternion):
        ...
        return yaw
    move_pose = [mouse_base_3d_points[1]+np.array([-0.03,-0.005,0.042])+np.array([0,0,new_red_gripper]), standard_quaternion] 
    D_pre = 0.05
    ...
    cur_js = bestman.robot.current_joint_state.position
    cur_js[-1] = -1.28576
    bestman.move_arm_to_joint_values(cur_js)
    print("SUCCESS------------------")
    ...
    preparation_position = [X - D_pre, 
                            ...
                            Z + D_pre]
    preparation_pose = Pose(preparation_position, place_orientation)
    force_move(bestman,preparation_pose, maxLinearVel=0.3, maxAngularVel=math.radians(120))
