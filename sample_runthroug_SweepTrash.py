...
for i in range(50):
    ...
    while True:
        try:
            grasp_pose = [mouse_base_3d_points[0]-np.array([-0.01,0.01,0.01])+np.array([0,0,new_red_gripper]), standard_quaternion] 
    ...
    ##########################sweep Pose##########################
    sweep_pose1 = [mouse_base_3d_points[1]+np.array([-0.02,-0.01,0.24])+np.array([0,0,new_red_gripper]), standard_quaternion] 
    ...
    sweep_pose2[0] += np.array([0.03,0,-0.07])
    sweep_pose2[1] = np.array([0.715368, 0.681405, -0.0599808, -0.142613])
    ...
    sweep_pose3[0] += np.array([0.1,0,-0.05])
