...
if USE_SAM2:
    ...
    # cv2.imwrite("/home/liusong/ProgramFiles/BestMan/Dataset/Images/MugRack.png",img)
    # cv2.imshow("img",img)
    # cv2.waitKey(0)


    frame = cv2.imread("/home/liusong/ProgramFiles/BestMan/Dataset/Images/MugRack.png")
    ...
    points = np.array([[459, 287],[468,344],[434,315]], dtype=np.float32)
    ...
    points = np.array([[329, 318],[303,337],[321,330]], dtype=np.float32)
    labels = np.array([1,1,1], dtype=np.int32)
    _, out_obj_ids, out_mask_logits = predictor.add_new_prompt(
        frame_idx=ann_frame_idx, obj_id=ann_obj_id, points=points, labels=labels
    )
