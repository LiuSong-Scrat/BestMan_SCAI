...
def ineraction_policy_inference(camera_hand,camera_overhead, bestman,stage1segmentation,stage2editing,predictor,model_savg,model_va,allow_gripper_open_flag):
...
def mission_execution(camera_hand,camera_overhead, bestman,stage1segmentation,stage2editing,predictor,model_savg,model_va):
    ...
    # visualize=True
    # cur_model_observation = get_cur_model_observation(camera_hand,camera_overhead, bestman)
    # target_pose_position,target_pose_orientation_xyzw = savg_inference(model_savg,cur_model_observation,predictor,stage1segmentation,obj_sets,has_cond,visualize=visualize)
    ...
    # #Interaction Phase
    # allow_gripper_open_flag = ineraction_policy_inference(camera_hand,camera_overhead, bestman,stage1segmentation,stage2editing,predictor,model_savg,model_va,allow_gripper_open_flag)
...
def savg_inference(model_savg,cur_model_observation,predictor,stage1segmentation,obj_sets,has_cond,visualize):
    # Objects Segmentation 
...
if USE_SAM2:
    ...
    frame = cv2.imread("/home/liusong/ProgramFiles/BestMan/Dataset/Images/PutStationeryBox.png")
    ...
    points = np.array([[308,359],[304,396],[300,437]], dtype=np.float32)
    ...
    points = np.array([[477, 330],[481,354],[485,387]], dtype=np.float32)
    labels = np.array([1,1,1], dtype=np.int32)
    _, out_obj_ids, out_mask_logits = predictor.add_new_prompt(
        frame_idx=ann_frame_idx, obj_id=ann_obj_id, points=points, labels=labels
    )
